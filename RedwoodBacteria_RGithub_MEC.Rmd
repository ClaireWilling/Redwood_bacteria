...
GDMaet<-function(){plot(rootsplines[[1]][,"aet"], rootsplines[[2]][,"aet"], type="l", 
lwd=3, xlab="Actual evapotranspiration (mm)",ylim=c(0,0.3),ylab="Partial Ecological Distance", tck=.02, las=1)
   points(soilsplines[[1]][,"aet"], soilsplines[[2]][,"aet"], col="darkgray", type="l", lwd=3)
   legend("topleft", legend = c("Root", "Soil"), col=c("black", "darkgray"), lwd=3, cex=0.8, box.lty=0)
   par(ps = 14, cex = 1, cex.main = 1)}
GDMaet<-ggdraw(GDMaet)
  

GDMph<-function() {plot(rootsplines[[1]][,"pH"], rootsplines[[2]][,"pH"], type="l", col="black", 
lwd=3, xlab="Soil pH", ylim=c(0,1.0),ylab="Partial Ecological Distance", tck=.02, las=1)
  points(soilsplines[[1]][,"pH"], soilsplines[[2]][,"pH"], type="l", col="darkgray",ylim=c(0,1.0),
lwd=3)
   legend("topleft", legend = c("Root", "Soil"), col=c("black", "darkgray"), lwd=3, cex=0.8, box.lty=0)
   par(ps = 14, cex = 1, cex.main = 1)}
GDMph<-ggdraw(GDMph)
...
model3c<-lmer(formula = Simpson~aet+scale(pH) +(1|SiteID/Treenum), data = RichnessROOTS, REML = T)
##Check boundaries
...
Simpsonroots<-ggplot(data=RichnessROOTS, aes(x=aet, y=Simpson))+
  stat_summary(fun.data=mean_sdl, fun.args = list(mult=1), geom="errorbar", 
        color="black") +
  stat_summary(fun=mean, geom="point", color="black", size=2)+
  labs(x="AET (scaled)", y="Simpson Diversity")+
  geom_abline(aes(intercept=1.006e+00, slope=-6.258e-05), color="black", linetype = "dashed")+
  ylim(0.940, 0.997)+
...
Shannonroots<-ggplot(data=RichnessROOTS, aes(x=scale(aet), y=Shannon))+
  ...
  stat_summary(fun=mean, geom="point", color="black", size=2)+
  labs(x="AET (scaled)", y="Shannon Diversity")+
...
Stepto<-dplyr::filter(fulldataclus2, OTU=="2")

Bradymoda<-lmer(formula = Abundance~scale(aet)*SampleType +(1|SiteID/Treenum), data = Brady, REML = F)
Bradymodb<-lmer(formula = Abundance~scale(aet)+SampleType +(1|SiteID/Treenum), data = Brady, REML = F)
Bradymodc<-lmer(formula = Abundance~scale(aet)*SampleType*scale(pH) +(1|SiteID/Treenum), data = Brady, REML = F)
Bradymodc<-lmer(formula = Abundance~scale(aet)*SampleType+(1|SiteID), data = Brady, REML = F)
Bradymodd<-lmer(formula = Abundance~scale(aet)*SampleType*scale(pH) +(1|SiteID/Treenum), data = Brady, REML = F)
Bradymode<-lmer(formula = Abundance~scale(aet)*SampleType+scale(pH) +(1|SiteID/Treenum), data = Brady, REML = F)
Bradymodf<-lmer(formula = Abundance~scale(aet)*SampleType*scale(pH) +(1|SiteID), data = Brady, REML = F)
...
Bradymodf<-lmer(formula = Abundance~scale(aet)*SampleType*scale(pH) +(1|SiteID), data = Brady, REML = T)

summary(Bradymodf)

#strepto
streptomoda<-lmer(formula = Abundance~scale(aet)*SampleType +(1|SiteID/Treenum), data = Stepto, REML = F)
streptomoda1<-lmer(formula = Abundance~scale(aet)*SampleType +(1|SiteID), data = Stepto, REML = F)
streptomodb<-lmer(formula = Abundance~scale(aet)+SampleType +(1|SiteID/Treenum), data = Stepto, REML = F)
streptomodc<-lmer(formula = Abundance~scale(aet)*scale(pH) +(1|SiteID/Treenum), data = Stepto, REML = F)
streptomodc1<-lmer(formula = Abundance~scale(aet)+scale(pH) +(1|SiteID/Treenum), data = Stepto, REML = F)
streptomodd<-lmer(formula = Abundance~scale(aet)*SampleType*scale(pH) +(1|SiteID/Treenum), data = Stepto, REML = F)
streptomode<-lmer(formula = Abundance~scale(aet)*SampleType+scale(pH) +(1|SiteID/Treenum), data = Stepto, REML = F)
streptomodf<-lmer(formula = Abundance~scale(aet)*SampleType*scale(pH) +(1|SiteID), data = Stepto, REML = F)
streptomoda<-lmer(formula = Abundance~scale(aet)*SampleType +(1|SiteID/Treenum), data = Stepto, REML = F)
...
##lowest is streptomoda1
streptomoda1<-lmer(formula = Abundance~scale(aet)*SampleType +(1|SiteID), data = Stepto, REML = T)

summary(streptomoda1)
...
##Selecting Model1c; 1A and 1C are the lowerst scores (within 2 points for AIC and BIC; 1C has less parameters, so selecting this as the most parsimonious model (no interaction term for Sample Type and does not include soil pH; signifcant correlation between Actino abundance and sample type and Actino abundance and AET)
...
model1a<-lmer(formula = Amycetalessums~scale(aet)*SampleType +(1|SiteID/Treenum), data = Amycetalesfulldf, REML = F)

model1b<-lmer(formula = Amycetalessums~SampleType+ scale(aet)*scale(pH) +(1|SiteID/Treenum), data = Amycetalesfulldf, REML = F)

model1c<-lmer(formula = Amycetalessums~scale(aet)+SampleType +(1|SiteID/Treenum), data = Amycetalesfulldf, REML = F)

model1d<-lmer(formula = Amycetalessums~SampleType*scale(aet)*scale(pH) +(1|SiteID/Treenum), data = Amycetalesfulldf, REML = F)

model1e<-lmer(formula = Amycetalessums~SampleType*scale(aet)*scale(pH) +(1|SiteID), data = Amycetalesfulldf, REML = F)
...
PhylumOrderdata<-full_join(Actinosumm, Amycetalessumm)
...
Actinoplot<-ggplot(PhylumOrderdata, aes(x=scale(aet), y = Meansumm, color=SampID, fill=SampleType)) +
  ...
  geom_abline(aes(intercept=coefAmyc[1], slope=coefAmyc[2]), data=as.data.frame(coefAmyc), color="#FF9900")+
  geom_abline(aes(intercept=coefAmyc[1]+coefAmyc[3], slope=coefAmyc[2]+coefAmyc[4]), data=as.data.frame(coefAmyc), color="#8BB8E8")+
  ...
  scale_color_manual(values = c("Actinobacteria_Root" = "#BE5504", "Actinobacteria_Soil"="#246bb5", "Actinomycetales_Root"="#FF9900", "Actinomycetales_Soil"="#8BB8E8"), labels=c("Actinobacteria (Root)", "Actinobacteria (Soil)", "Actinomycetales (Root)", "Actinomycetales (Soil)"), name="Group")+
  theme(
            text = element_text(family = "Helvetica"),
            plot.title = element_text(margin = margin(b=5, r=5, l=5, unit = "mm"),
                                      size = rel(1),
...
Chlorpflexidf<-filter(Chloroflexidf, Phylum=="Chloroflexi")
...
##Full model (both soil and root data; sample type*aet interaction)
Chloromodel1a<-lmer(formula = Chloroflexisums~scale(aet)*SampleType +(1|SiteID/Treenum), data = Chloroflexifulldf, REML = F)

Chloromodel1b<-lmer(formula = Chloroflexisums~SampleType+ scale(aet)*scale(pH) +(1|SiteID/Treenum), data = Chloroflexifulldf, REML = F)

Chloromodel1c<-lmer(formula = Chloroflexisums~scale(aet)+SampleType +(1|SiteID/Treenum), data = Chloroflexifulldf, REML = F)

Chloromodel1d<-lmer(formula = Chloroflexisums~SampleType*scale(aet)*scale(pH) +(1|SiteID/Treenum), data = Chloroflexifulldf, REML = F)

Chloromodel1e<-lmer(formula = Chloroflexisums~SampleType*scale(aet)*scale(pH) +(1|SiteID), data = Chloroflexifulldf, REML = F)
...
##Selecting ChloroModel1e (lowest AIC and BIC)
Chloromodel1e<-lmer(formula = Chloroflexisums~SampleType*scale(aet)*scale(pH) +(1|SiteID), data = Chloroflexifulldf, REML = T)
qqp(residuals(Chloromodel1e), col=2)

##Visual inspection of residuals--all look good (no outliers)

summary(Chloromodel1e)
...
Firmicutesmodel1a<-lmer(formula = Firmicutessums~scale(aet)*SampleType +(1|SiteID/Treenum), data = Firmicutesfulldf, REML = F)

Firmicutesmodel1b<-lmer(formula = Firmicutessums~SampleType+ scale(aet)*scale(pH) +(1|SiteID/Treenum), data = Firmicutesfulldf, REML = F)

Firmicutesmodel1c<-lmer(formula = Firmicutessums~scale(aet)+SampleType +(1|SiteID/Treenum), data = Firmicutesfulldf, REML = F)

Firmicutesmodel1d<-lmer(formula = Firmicutessums~SampleType*scale(aet)*scale(pH) +(1|SiteID/Treenum), data = Firmicutesfulldf, REML = F)

Firmicutesmodel1e<-lmer(formula = Firmicutessums~SampleType*scale(aet)*scale(pH) +(1|SiteID), data = Firmicutesfulldf, REML = F)

Firmicutesmodel1f<-lmer(formula = Firmicutessums~SampleType +(1|SiteID), data = Firmicutesfulldf, REML = F)
...
##Selecting Firmicutesmodel1a 
Firmicutesmodel1a<-lmer(formula = Firmicutessums~scale(aet)*SampleType +(1|SiteID/Treenum), data = Firmicutesfulldf, REML = T)

library(car)
qqp(residuals(Firmicutesmodel1a), col=2)

summary(Firmicutesmodel1a)

```

```{r message=FALSE, warning=FALSE}
setwd("~/Dropbox/Research/Thesis_Ch3/Manuscript/Molecular Ecology Submission/Resubmission/")
...
setwd("~/Dropbox/Research/Thesis_Ch3/Manuscript/Molecular Ecology Submission/Resubmission/")
#ggsave("Figure1AB.pdf", plot = Figure1AB, width = 10.2, height = 5) 

Figure2<-cowplot::plot_grid(GDMph, GDMaet,
                  labels = c("a)", "b)"), 
                  label_size = 14,
                  vjust=4.8,
                  label_fontfamily = "sans")
setwd("~/Dropbox/Research/Thesis_Ch3/Manuscript/Molecular Ecology Submission/Resubmission/")
#ggsave("Figure2.pdf", plot = Figure2, width =8 , height = 3.85) 


Figure3be<-cowplot::plot_grid(Shannonroots, Shannonsoils,Simpsonroots,Simpsonsoils,
                   labels = c("b)", "c)", "d)", "e)"), 
                   label_size = 12, 
                   label_fontfamily = "sans")

Figure3a<-cowplot::plot_grid(Actinoplot, 
                            #rel_heights = c(1.85, 0.1),
                            ncol=1,
                            labels = c("a)"),
                            label_fontfamily = "sans")

Figure3AB<-cowplot::plot_grid(Figure3a,Figure3be,
                            rel_heights = c(2.2,2),
                            ncol=1,
                            label_size = 12, 
                            label_fontfamily = "sans")
setwd("~/Dropbox/Research/Thesis_Ch3/Manuscript/Molecular Ecology Submission/Resubmission/")
#ggsave("Figure3AB.pdf", plot = Figure3AB, width =8 , height = 9.1) 

Figure4<-cowplot::plot_grid(RANKFULLplot)
setwd("~/Dropbox/Research/Thesis_Ch3/Manuscript/Molecular Ecology Submission/Resubmission/")
#ggsave("Figure4.pdf", plot = Figure4, width =12 , height = 8) 


Figure5<-cowplot::plot_grid(Toptaxaplot)
  
setwd("~/Dropbox/Research/Thesis_Ch3/Manuscript/Molecular Ecology Submission/Resubmission/")
#ggsave("Figure5.pdf", plot = Figure5, width =6 , height = 4) 

Diversitymodeltable<-tab_model(model3c, model3d.Shannon, model4c, model4c.shannon, auto.label = F)
Diversitymodeltable 

ModeltableActino<-tab_model(model1c,model1aAmycetales, auto.label = F)
ModeltableActino

OTUmodeltab<-tab_model(Bradymodf, streptomoda1, auto.label = F)
OTUmodeltab

ChloroFirmicutestab<-tab_model(Firmicutesmodel1a, Chloromodel1e, auto.label = F)
ChloroFirmicutestab
